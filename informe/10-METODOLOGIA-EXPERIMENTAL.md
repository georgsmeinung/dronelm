...
La evaluación compara tres brazos de decisión sobre el mismo `ObstacleField` (cap. 6), el mismo
espacio de macro-acciones y el mismo traductor a comandos cinemáticos (`action_to_command`, cap. 8):

- **`slm`** — el modelo de lenguaje pequeño como capa de decisión táctica (cap. 8), con freno previo,
...
comparación estadística (§10.3) requiere varianza entre semillas para ser significativa. El presupuesto
...
ninguna misión por la cadena de fallas descrita en el capítulo 9 (ya corregida en la arquitectura
...
- `deliberation_rate` (fracción de ciclos que efectivamente invocan al modelo de lenguaje) y el
  ...
  excepción (cap. 5) se sostiene en la práctica o si, por el contrario, el modelo termina siendo
...
- Invocaciones del SLM por misión, tasa de *fallback* al respaldo determinista, tasa de *timeout* del
  watchdog.
- `adherence_rate`, con y sin decodificación restringida (§8.2).
...
reproducible, con escenarios y semillas fijos. El conjunto de datos de validación de TTC (cap. 7) y
